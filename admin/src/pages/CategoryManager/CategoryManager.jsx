...
import { Grid,Box, TextField, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, Dialog, DialogTitle, DialogContent, IconButton, Pagination } from '@mui/material';
...
const CategoryManager = () => {
  ...
  const getStock = (id) => {
      ...
      if (Array.isArray(products)) {
          stock = products
              ...
              .reduce((total, item) => total + (getStockOfProduct(item.id) || 0), 0); 
  ...
  const handleDeleteCategory = async (productId) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/Categories/${productId}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        setCategories(categories.filter(product => product.id !== productId));
        alert('Danh muc đã được xóa thành công');
      } else {
  ...
  const filteredCategories = categories.filter((categories) =>
    categories.name.toLowerCase().includes(searchTerm.toLowerCase()) || categories.id.includes(searchTerm)
  );
  ...
  return (
    <Box padding={3}>
      ...
      <Box marginY={2}>
        <Button variant="contained" color="primary" onClick={handleAddCategory}>Thêm sản phẩm</Button>
      </Box>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ width: '220px' }}>Tên danh mục</TableCell>
          ...
          <TableBody>
            {paginatedCategories.map((category) => (
              <TableRow key={category.id}>
                ...
                <TableCell>
                  {getStock(category.id) > 0 ? getStock(category.id) : (<p style={{color:"red"}}>Hết hàng</p>)}
                </TableCell>
                <TableCell>
                  <IconButton onClick={() => handleViewDetails(category)} color="primary">
                    <Visibility />
                  </IconButton>
                  <IconButton onClick={() => handleEditCategory(category.id)} color="secondary">
                    <Edit />
                  </IconButton>
        ...
        >
          ...
          <DialogContent sx={{ padding: '24px' }}>
            <Box display="flex" gap={3}>
              <Box flex={1}>
                <img
                  src={`data:image/jpeg;base64,${selectedCategory?.image}`|| ""}
              ...
              <Box flex={1}>
                ...
                <Box sx={{ 
                  ...
                  <Typography>
                    <strong>Mã danh mục:</strong> {selectedCategory.id}
                  </Typography>
                  <Typography>
                  ...
                  <Grid>
                  {getProduct(selectedCategory.id)
                    .map((pro) => {
                      return (
                        <Typography
                          key={pro.id}
                        ...
                        >
                          <img
                            src={`data:image/jpeg;base64,${pro?.image}` || ""}
                            alt={selectedCategory.name}
                            style={{
                              width: '40px',
                              height: 'auto',
                          ...
                          />
                          <Link to={`/edit-product/${pro.id}`}>{pro.name}</Link>
                        </Typography>
                      );
                    })}
                </Grid>
