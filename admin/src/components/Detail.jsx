...
const fetchDetail = async (productId) => {
  const response = await axios.get(`${API_URL}/api/Details/ProductDetail/${productId}`);
  return response.data;
};

// API cập nhật chi tiết sản phẩm
const updateDetail = async (updatedDetail) => {
  const response = await axios.put(`${API_URL}/api/Details/${updatedDetail.id}`, updatedDetail);
  return response.data;
};
      ...
      {detailKeys.map((key) => (
        <Grid item xs={10} sm={6} key={key}>
          <TextField
            ...
            fullWidth
            margin="normal"
            onChange={(e) => setEditedDetail({ ...editedDetail, [key]: e.target.value })}
          />
        </Grid>
      ))}
      ...
      <Grid item xs={10} sm={6} mt={3}>
        {details.length > 0 ? (
          <Button
            variant="contained"
            color="primary"
            onClick={() => updateMutation.mutate({ ...details[0], ...editedDetail })}
            disabled={updateMutation.isLoading}
            size="large"
          >
            {updateMutation.isLoading ? "Đang cập nhật..." : "Cập nhật cấu hình sản phẩm"}
          </Button>
        ) : (
          <Button
            variant="contained"
            color="primary"
            onClick={() => addMutation.mutate({ productId, ...editedDetail })}
            disabled={addMutation.isLoading}
            size="large"
          >
            {addMutation.isLoading ? "Đang thêm..." : "Thêm cấu hình sản phẩm"}
          </Button>
        )}
      </Grid>
    </Grid>
